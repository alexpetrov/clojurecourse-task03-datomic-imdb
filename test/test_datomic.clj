...
(deftest test-siblings
  (println "test siblings :movie :videogame")
  (time (is (= (siblings (db) :movie :videogame)
  ...
  (println "test siblings :series :videogame")
  (time (is (= (siblings (db) :series :videogame)
...
(deftest test-oldest-series
  (println "test-oldest-series")
  (time (is (= (oldest-series (db))
         #{["\"Today\" (1952)" 1952]}))))

(deftest test-longest-season
  (println "test-longest-season")
  (time (is (= (longest-season (db))
         [["\"Familie\" (1991)"           1 88]
...
(deftest test-popular-titles
  (println "test-popular-titles")
  (time (is (= (popular-titles (db))
         [[9 "Alone"]
          [8 "Home"]
          [6 "Broken"]
          [6 "Destiny"]
